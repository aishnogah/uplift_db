...
class SpatialDBImpl : public SpatialDB {
 public:
  ...
  virtual Status BulkInsert(const WriteOptions& write_options,
          const std::vector<Coord<double>>& coords,
          const std::vector<Slice>& blobs,
          const std::vector<FeatureSet>& feature_sets) {
    WriteBatch batch;
    Statistics* stats = db_->GetOptions().statistics.get();
    Env* env = GetEnv();

    size_t n = coords.size();
    if (n == 0 || blobs.size() != n || feature_sets.size() != n) {
      return Status::InvalidArgument("coords, blobs, and feature_sets must have same size >0");
    }
    if (!zoom_levels_) {
      return Status::InvalidArgument("BulkInsert only valid for zoom_levels spatial db");
    }

//    const size_t kWriteOutEveryBytes = 1024 * 1024;  // 1MB
    std::vector<uint64_t> ids(n);
    for (size_t i=0; i<n; ++i) {
      ids[i] = next_id_.fetch_add(1);
    }
    std::vector<Coord<uint64_t>> tiles(n);
    ...
    for (size_t i=1; i<= zoom_levels_; ++i) {
      ...
      spatial_indexes.push_back(si);
    }
    const auto& deepest = spatial_indexes.back();

    for (size_t i=0; i<n; ++i) {
      Coord<uint64_t> tile = GetTile(deepest.index, coords[i]);
      quadkeys[i] = GetQuadKeyFromTile(tile.x, tile.y, deepest.index.tile_bits);
    }
...

    {
      ...
      for (uint32_t zoom = 1; zoom <= zoom_levels_; ++zoom) {
        ...
        for (size_t i = 0; i < n; ++i) {
          size_t j = permute[i];
          uint64_t quad = quadkeys[j];
          uint64_t id = ids[j];
          FeatureSet feature_set = feature_sets[j];

          std::string key;
          uint64_t shifted_quad = quad >> shift;
          PutFixed64BigEndian(&key, shifted_quad);
          PutFixed64BigEndian(&key, id);
          batch.Put(si.column_family, key, Slice());
        }
    ...
    for (size_t i=0; i<n; ++i) {
      //Do this in id order
      std::string data_key;
      PutFixed64BigEndian(&data_key, ids[i]);
      std::string data_value;
      PutLengthPrefixedSlice(&data_value, blobs[i]);
      feature_sets[i].Serialize(&data_value);
 ...
 private:
  ...
  bool read_only_;
  const uint32_t zoom_levels_;
};

namespace {

ColumnFamilyOptions GetColumnFamilyOptions(std::shared_ptr<Cache> block_cache) {
...
class FixedRangeTransform : public SliceTransform {
private:
  size_t range_begin_;    // inclusive
  size_t range_end_;      // exclusive
  size_t len_;            // range_end_ - range_begin_
  std::string name_;

public:
  explicit FixedRangeTransform(size_t range_begin, size_t range_end)
          : range_begin_(range_begin),
            range_end_(range_end),
            len_(range_end - range_begin),
            name_("Ben.FixedRange." + std::to_string(range_begin_) + "-" + std::to_string(range_end_))
  {}
  ...
  virtual Slice Transform(const Slice& src) const {
    assert(InDomain(src));
    return Slice(src.data()+range_begin_, len_);
  }

  virtual bool InDomain(const Slice& src) const {
    return (src.size() >= range_end_);
  }
...
ColumnFamilyOptions OptimizeOptionsForSpatialColumnFamily
        (ColumnFamilyOptions options, size_t zoom, bool ram_disk) {
  if (ram_disk) {
    PlainTableOptions table_options;
    table_options.user_key_len = sizeof(uint64_t) * 2;
    options.table_factory.reset(NewPlainTableFactory(table_options));

    // Set prefix extractor
    size_t bytes_needed = (2U * zoom + CHAR_BIT - 1U) / CHAR_BIT; // ceiling of (2*zoom / 8)
    size_t range_begin = sizeof(uint64_t) - bytes_needed;
    options.prefix_extractor.reset(
            NewFixedRangeTransform(range_begin, sizeof(uint64_t)));

    size_t bucket_count = std::min(500000UL, 10L * pow(4, zoom));
...
ColumnFamilyOptions OptimizeOptionsForDataColumnFamily(
    ColumnFamilyOptions options, std::shared_ptr<Cache> block_cache, bool ram_disk = false) {
  ...
  if (!ram_disk) {
    ...
    PlainTableOptions table_options;
    table_options.user_key_len = sizeof(uint64_t);
    options.table_factory.reset(NewPlainTableFactory(PlainTableOptions(table_options)));
  }
  return options;
}

ColumnFamilyOptions OptimizeOptionsForMetadataColumnFamily(
        ColumnFamilyOptions options, std::shared_ptr<Cache> block_cache, bool ram_disk = false) {
...
Status SpatialDB::Create(
        DBOptions& db_options, const std::string& name,
        BoundingBox<double> outer_box, uint32_t zoom_levels, bool ram_disk) {
  ...
  auto block_cache = NewLRUCache(cache_size);
  ColumnFamilyOptions column_family_options = GetColumnFamilyOptions(block_cache);

  std::vector<ColumnFamilyDescriptor> column_families;

  column_families.emplace_back(kDefaultColumnFamilyName,
          OptimizeOptionsForDataColumnFamily(column_family_options, block_cache, ram_disk));

  column_families.emplace_back(kMetadataColumnFamilyName,
  ...
  for (uint32_t zoom = 1; zoom <= zoom_levels; ++zoom) {
    ...
    column_families.emplace_back(GetSpatialIndexColumnFamilyName(index_name),
            OptimizeOptionsForSpatialColumnFamily(column_family_options, zoom, ram_disk));
  }
...
Status SpatialDB::Open(
        DBOptions& db_options, const std::string& name, SpatialDB** db,
        bool read_only, uint32_t zoom_levels, bool ram_disk) {
  ...
  assert(handles.size() == spatial_index_names.size() + 2);
  for (size_t i = 0; i < spatial_index_names.size(); ++i) {
    SpatialIndexOptions index_options;
    s = metadata.GetIndex(spatial_index_names[i], &index_options);
    if (!s.ok()) {
      break;
    }
  ...
  delete handles[1];
  *db = new SpatialDBImpl(base_db, handles[0], index_cf, next_id, read_only, zoom_levels);
  return Status::OK();
}
