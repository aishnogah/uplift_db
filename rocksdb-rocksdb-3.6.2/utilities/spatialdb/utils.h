...
inline Coord<uint64_t> GetTile(
        const SpatialIndexOptions& spatial_index, Coord<double> point) {
  return Coord<uint64_t>(
      GetTileFromCoord(point.x, spatial_index.bbox.min_x,
                       spatial_index.bbox.max_x, spatial_index.tile_bits),
      GetTileFromCoord(point.y, spatial_index.bbox.min_y,
                       spatial_index.bbox.max_y, spatial_index.tile_bits));
}
...
inline bool GetFixed32BigEndian(const Slice& input, uint32_t* value) {
  ...
  auto ptr = input.data();
  *value = (static_cast<uint64_t>(static_cast<unsigned char>(ptr[0])) << 24) |
           (static_cast<uint64_t>(static_cast<unsigned char>(ptr[1])) << 16) |
           (static_cast<uint64_t>(static_cast<unsigned char>(ptr[2])) << 8) |
            static_cast<uint64_t>(static_cast<unsigned char>(ptr[3]));
  return true;
}

// big endian can be compared using memcpy
inline void PutFixed16BigEndian(std::string* dst, uint16_t value) {
  char buf[sizeof(value)];
  buf[0] = (value >> 8) & 0xff;
  buf[1] = value & 0xff;
  dst->append(buf, sizeof(buf));
...
// big endian can be compared using memcpy
inline bool GetFixed16BigEndian(const Slice& input, uint16_t* value) {
  if (input.size() < sizeof(uint16_t)) {
    return false;
  }
  auto ptr = input.data();
  *value = (static_cast<uint64_t>(static_cast<unsigned char>(ptr[0])) << 8) |
