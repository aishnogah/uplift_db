...
struct BoundingBox {
  ...
  bool operator==(const BoundingBox<T>& bbox) const {
    return (min_x == bbox.min_x && min_y == bbox.min_y && max_x == bbox.max_x && max_y == bbox.max_y);
  }
};

template <typename T>
struct Coord : BoundingBox<T> {
  T x, y;
  Coord() = default;
  Coord(T _x, T _y) : BoundingBox<T>(_x, _y, _x, _y), x(_x), y(_y) {}
...
class SpatialDB : public StackableDB {
 public:
  ...
  static Status Create(DBOptions& options, const std::string& name,
                       BoundingBox<double> outer_box, uint32_t zoom_levels,
                       bool ram_disk = false);
  ...
  static Status Open(DBOptions& options, const std::string& name, SpatialDB** db,
                     bool read_only, uint32_t zoom_levels, bool ram_disk = false);
  ...
  // Bulk insert Coords into the DB. Elements will be inserted into specified
  // spatial_indexes, based on specified bbox.
  // REQUIRES: spatial_indexes.size() > 0
  virtual Status BulkInsert(const WriteOptions& write_options,
          ...
          const std::vector<FeatureSet>& feature_sets) = 0;
